...
import { isClass, isClassObject, makeClass, makeClassObject, ClassObject, isSymbolSExp } from "./L3-value"; // <====
import { isClassExp, isCExp, isLetExp } from "./L3-ast";
...
const L3applyProcedure = (proc: Value, args: Value[], env: Env): Result<Value> =>
    ...
    isClassObject(proc) ? applyClassObject(proc, args, env) :    // <====
...
const applyClassObject = (obj: ClassObject, args: Value[], env: Env): Result<Value> => {
    ...
    const vars = map((v: VarDecl) => v.var, obj.class.fields);
    const litArgs: CExp[] = map(valueToLitExp, obj.fieldVals);
    const substitutedMethodBody = substitute([method.val], vars, litArgs);
    return bind(L3applicativeEval(substitutedMethodBody[0], env), (methodVal: Value) =>
