...
export const parseL3SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>
    ...
    op === "class" ? // <====
        ...
        makeFailure(`Bad class exp: ${format(params)}`)) :
    makeFailure("Never");
...
const parseClassExp = (fields: Sexp, methods: Sexp): Result<ClassExp> => { // <====
    ...
    if (!isArray(methods) || !isGoodBindings(methods)) {
    ...
    return bind(valsResult, (vals: CExp[]) => 
        makeOk(makeClassExp(map(makeVarDecl, fields), zipWith(makeBinding, vars, vals)))
    );
};
